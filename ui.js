...
// Layer buttons/display references removed
export const isolateLayerBtn = document.getElementById('isolateLayerBtn'); // Keep isolation toggle button ref
export const currentLayerInfo = document.getElementById('currentLayerInfo'); // Keep info span ref
...
export function populatePalette() {
    ...
    for(const category in config.TILE_CATEGORIES){const catDiv=document.createElement('div');catDiv.className='tile-category'; const title=document.createElement('h3');title.textContent=category;catDiv.appendChild(title);
        config.TILE_CATEGORIES[category].forEach(tileId=>{ const span=document.createElement('span'); span.dataset.tile=tileId; span.title=`Build ${tileId}`;
            if(config.GENERATED_TILE_IDS.has(tileId)){tempCtx.clearRect(0,0,tempCanvasSize,tempCanvasSize); const cT=tempCtx.getTransform(); tempCtx.setTransform(1,0,0,1,0,0); drawGeneratedTile(tempCtx,0,0,tempCanvasSize,tileId); tempCtx.setTransform(cT); span.style.backgroundImage=`url(${tempCanvas.toDataURL()})`; span.style.backgroundSize='cover'; span.textContent='';} // Clear text
            else{span.style.backgroundImage='none'; span.textContent=tileId;}
...
export function selectTile(tileId) {
    ...
    // No need to call selectLayer here anymore

    // Update visual selection in palette
    const spans=paletteDiv.querySelectorAll('span'); spans.forEach(span=>{span.classList.toggle('selected-tile',span.dataset.tile===tileId);});
    ...
    // Update the displayed current placement layer info span
    if (currentLayerInfo) {
        currentLayerInfo.textContent = `Layer: ${state.currentPlacementLayer}`;
    }
}
...
export function updateSizeUI() {
    ...
    if (sizeSlider.value !== String(size)) {
...
// updateLayerUI function removed (replaced by logic within selectTile and updateIsolationDisplay)
...
export function updateToolUI() {
    selectedTileDisplay.style.display = state.currentTool === 'build' ? 'inline-block' : 'none';
