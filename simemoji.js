...
// Correct import: Use getEffectiveTileInfo
...
// isolatedLayer variable removed - derive from state.currentPlacementLayer when isolation is ON
...
function addEventListeners() {
    ...
    // Tools, Size, Save/Load/Clear, Zoom, Undo/Redo listeners... (Unchanged)
    ...
    // Layer Isolation Button Listener ADDED
    const isolateBtn = document.getElementById('isolateLayerBtn'); // Get button ref
    if(isolateBtn) { // Check if button exists
         isolateBtn.addEventListener('click', toggleLayerIsolation);
    } else {
        console.error("Isolate Layer button not found!");
    }
    ...
    // State Loaded Listener... (Unchanged)
...
function toggleLayerIsolation() {
    ...
    updateIsolationDisplay(); // Update button style and info span
...
function updateIsolationDisplay() {
    ...
    if (isolateBtn) {
        isolateBtn.classList.toggle('isolation-active', isLayerIsolationActive); // Use CSS class for active state
    ...
    if (layerInfoSpan) {
        // Show the layer that WILL BE placed (derived from selected tile)
        layerInfoSpan.textContent = `Layer: ${state.currentPlacementLayer}`;
         // Optionally add tooltip indicating isolation state
         if (isLayerIsolationActive) {
             layerInfoSpan.title = `Isolation ON (Layer ${state.currentPlacementLayer})`;
             ...
             layerInfoSpan.style.color = '#ccc'; // Reset color
         }
    }
     // Also update panel border as another indicator
     const uiPanel = document.getElementById('uiPanel');
     if(uiPanel) {
         uiPanel.style.borderLeft = isLayerIsolationActive ? '3px solid yellow' : 'none';
     }
}
...
function handleMouseMove(e) { /* ... Unchanged ... */ const canvas=document.getElementById('gameCanvas'); const rect=canvas.getBoundingClientRect(); mouseScreenX=e.clientX-rect.left; mouseScreenY=e.clientY-rect.top; mouseWorldX=(mouseScreenX/state.zoomLevel)+state.camX; mouseWorldY=(mouseScreenY/state.zoomLevel)+state.camY; mouseGridX=Math.floor(mouseWorldX/config.BASE_GRID_SIZE); mouseGridY=Math.floor(mouseWorldY/config.BASE_GRID_SIZE); ui.updateCoordsDisplay(mouseGridX,mouseGridY); if(isPanning){ const dx=(mouseScreenX-panStartX)/state.zoomLevel; const dy=(mouseScreenY-panStartY)/state.zoomLevel; state.setCamPos(panStartCamX-dx,panStartCamY-dy); clampCamera(); } }
...
function handleMouseDown(e) {
    ...
    if (e.button === 0) {
        ...
        // --- Layer Placement Rule Check --- (Unchanged)
        ...
        if (tool === 'bulldoze') {
            ...
            if (!objectToDelete || (isLayerIsolationActive && objectToDelete.layer !== state.currentPlacementLayer)) { console.log("Nothing to bulldoze on visible/target layer."); return; }

            const cellsToClear = []; const objectLayer = objectToDelete.layer; // Layer of the object being deleted
            for (let dx=0; dx<objectToDelete.size; dx++) { for (let dy=0; dy<objectToDelete.size; dy++) {
                ...
                if (!currentLayerMap || !currentLayerMap[objectLayer]) continue; // Nothing on this layer to delete

                const newLayerMap = { ...currentLayerMap }; // Clone
                delete newLayerMap[objectLayer]; // Remove item from its layer
        ...
        // Build Logic (Uses getEffectiveTileInfo)
            ...
            for (let dx=0; dx<size; dx++) { for (let dy=0; dy<size; dy++) {
                ...
                const newLayerMap = { ...(currentLayerMap || {}) };
                newLayerMap[layerToPlaceOn] = newDataForLayer;
...
function handleMouseUp(e) { /* ... Unchanged ... */ } function handleMouseOut() { /* ... Unchanged ... */ } function handleMouseWheel(e) { /* ... Unchanged ... */ }
function handleKeyDown(e) { // Removed layer change keys
    keys[e.code]=true; if(e.ctrlKey||e.metaKey){ if(e.code==='KeyZ'){e.preventDefault();state.undo();}else if(e.code==='KeyY'){e.preventDefault();state.redo();}else if(e.code==='KeyS'){e.preventDefault();state.saveGameToFile();}else if(e.code==='KeyO'){e.preventDefault();ui.fileInput.click();}} else { if(e.code==='KeyB')ui.setTool('build'); if(e.code==='KeyX')ui.setTool('bulldoze'); if(e.code==='Equal'||e.code==='NumpadAdd')zoom(1+config.ZOOM_INCREMENT); if(e.code==='Minus'||e.code==='NumpadSubtract')zoom(1/(1+config.ZOOM_INCREMENT)); if(e.code === 'KeyL') toggleLayerIsolation();} // Keep L for isolation
}
...
function updateCurrentGridSize(){currentGridSize=config.BASE_GRID_SIZE*state.zoomLevel;} function clampCamera(){/*...*/} function zoom(factor,pivotX,pivotY){/*...*/} function updatePanningInput(){/*...*/}

// --- Rendering --- (Unchanged from previous - already uses correct layer map logic)
function render() {
    ...
    for(let gx=startGridX;gx<endGridX;gx++){for(let gy=startGridY;gy<endGridY;gy++){ if(gx<0||gx>=config.WORLD_WIDTH_CELLS||gy<0||gy>=config.WORLD_HEIGHT_CELLS)continue; const key=`${gx},${gy}`; const layerMap=state.gridData.get(key); if(!layerMap)continue; const layers=Object.keys(layerMap).map(Number).sort((a,b)=>a-b);
        for(const layer of layers){ if(isLayerIsolationActive&&layer!==state.currentPlacementLayer)continue; const tileData=layerMap[layer]; let tileIdToRender=null; let itemSize=1; let isOrigin=false; let originX=gx; let originY=gy; let worldX=0,worldY=0,worldYBottom=0; let itemFontSize=baseTileFontSize; let isGenerated=false;
            if(typeof tileData==='string'){tileIdToRender=tileData;itemSize=1;isOrigin=true;originX=gx;originY=gy;}else if(typeof tileData==='object'&&tileData!==null){if(tileData.isOrigin){tileIdToRender=tileData.tile;itemSize=tileData.size||1;isOrigin=true;originX=gx;originY=gy;}else{const originKey=`${tileData.originX},${tileData.originY}`; const originLayerMap=state.gridData.get(originKey); const originData=originLayerMap?originLayerMap[layer]:null; if(originData?.isOrigin){tileIdToRender=originData.tile;itemSize=originData.size||1;isOrigin=false;originX=tileData.originX;originY=tileData.originY;}else{continue;}}}
    ...
    // Draw Preview... (Validity check adjusted)
